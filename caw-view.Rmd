...
```{r load_reference}
snptable=fread('~/reports/reference_data/private/swegen_snp_counts.csv',key='name')
swegen_manta_all=fread('~/reports/reference_data/private/swegen_sv_counts.csv',key='name')
...
if (exists('haplotypecaller_N_file')) {
  ...
  for (s in 1:length(haplotypecaller_files)) {
    ...
    mutations$AFreq=round(sapply(g$AD, "[[", 2) / unlist(g$DP),2) # not a great estimate...
    ad=as.data.table(g$AD)
    colnames(ad)='AD'
    mutations=cbind(mutations,ad)
    # mutations$reads=''
