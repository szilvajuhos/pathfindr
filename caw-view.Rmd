...
if (project == 'BTB') {
  ...
  write("-----------------------------------------",stderr());
  write("FREEC files",stderr());
  write(paste("freec_Tratio_file", freec_Tratio_file),stderr());
  write(paste("freec_Nratio_file", freec_Nratio_file),stderr());
  write(paste("freec_Tbaf_file",   freec_Tbaf_file),stderr());
  write(paste("freec_Nbaf_file",   freec_Nbaf_file),stderr());
  write(paste("freec_info_file",   freec_info_file),stderr());
  write(paste("freec_cnv_file",    freec_cnv_file ),stderr());
  # Ascat files
  ...
  write("-----------------------------------------",stderr());
  write("ASCAT files: ",stderr())
  write(paste("ascat_Tratio_file:",ascat_Tratio_file),stderr())
  write(paste("ascat_Nratio_file:",ascat_Nratio_file),stderr())
  write(paste("ascat_Tbaf_file:",ascat_Tbaf_file),stderr())
  write(paste("ascat_Nbaf_file:",ascat_Nbaf_file),stderr())
  write(paste("ascat_segment_file:",ascat_segment_file),stderr())
  ...
  write("-----------------------------------------",stderr());
  write("Manta files: ",stderr())
  write(paste("manta_tumor_file:",manta_tumor_file),stderr())
  write(paste("manta_normal_file:",manta_normal_file),stderr())
  # Haplotype caller variant files
  haplotypecaller_T_file <- grep(pattern = ".*VEP\\.CADD.*haplotypecaller.*[TR].*AF.*vep.ann.vcf$",files,value = T)
  # haplotypecaller_T_file <- 
  #   grep(pattern = "^.*Annotation/VEP/haplotypecaller.*02.*vcf$",files,value = T)[1]
  haplotypecaller_N_file <- grep(pattern = ".*VEP\\.CADD.*haplotypecaller.*[NB].*AF.*vep.ann.vcf$",files,value = T)
  write("-----------------------------------------",stderr());
  write(" HaplotypeCaller files: ",stderr())
  write(paste("haplotypecaller_N_file:",haplotypecaller_N_file),stderr())
  write(paste("haplotypecaller_T_file:",haplotypecaller_T_file),stderr())
  # haplotypecaller_N_file <- 
  #   grep(pattern = "^.*Annotation/VEP/haplotypecaller.*01.*vcf$",files,value = T)[1]
  # snpEff+VEP Mutect2 file
  mutect2_file <- grep(pattern = ".VEP\\.CADD.*mutect2_.*AF.*vep.ann.vcf$",files,value = T)
  write("-----------------------------------------",stderr());
  write(" MuTect2 files: ",stderr())
  write(paste("mutect2_file:",mutect2_file),stderr())
  # snpEff + VEP Strelka snv file
  strelka_snv_file <- grep(pattern = ".*VEP\\.CADD.*Strelka_.*_somatic_snvs.*AF.*vep.ann.vcf$",files,value = T)
  # snpEff + VEP Strelka indel file
  strelka_indel_file <- grep(pattern = "./*VEP\\.CADD.*Strelka_.*_somatic_indels.*AF.*vep.ann.vcf$",files,value = T)
  write("-----------------------------------------",stderr());
  write(" Strelka files: ",stderr())
  write(paste("strelka_snv_file:",strelka_snv_file),stderr())
  write(paste("strelka_indel_file:",strelka_indel_file),stderr())
}
...
if (!is.null(manta_tumor_table)) if (nrow(manta_tumor_table)>0) {
  ...
  if (nrow(selection)>0) {
    ...
    if (write_tables) fwrite(manta_tumor_selected[,-c('ANN','Gene_ID','CSQ')][rank_score>1],file=paste0(sampleData$name,'_manta_tumor.csv'))
...
if (project=='BTB') {
  ...
  if (!is.null(manta_normal_table)) if (nrow(manta_normal_table)>0) {
    ...
    if (nrow(selection)>0) {
      ...
      if (write_tables) fwrite(manta_normal_selected[,-c('ANN','Gene_ID')][rank_score>1],file=paste0(sampleData$name,'_manta_normal.csv'))
...
if (exists('haplotypecaller_N_file')) {
  ...
  for (s in 1:length(haplotypecaller_files)) {
    ...
    if (project=='BTB') {
      ...
      keep=unique(swaf$group[which(values$max<0.01)])  # variant IXs with some allele <1% in SWAF and TOPMED (swaf$group equals topmed$group)
      vcf=vcf[keep]                 
    } else {
      in_ref=snptable[names(vcf)]
      remove_swegen=which(in_ref$value>=10)
