...
# the tumor file
manta_tumor_file <- checkFileExistence(".*Manta_.*vs.*somaticSV.vcf.snpEff.ann.vep.ann.vcf$",files)
write(paste("Manta tumor file: ",manta_tumor_file),stdout())
# The Manta normal file
manta_normal_file <- checkFileExistence(".*Manta_.*vs.*diploidSV.vcf.snpEff.ann.vep.ann.vcf$", files)
write(paste("Manta normal file: ",manta_normal_file),stdout())

# Haplotype caller variant files
haplotypecaller_T_file <- checkFileExistence(".*haplotypecaller.*[TR].*AF.*vep.ann.vcf$",files)
write(paste("HC tumor file: ",haplotypecaller_T_file),stdout())
haplotypecaller_N_file <- checkFileExistence(".*haplotypecaller.*[NB].*AF.*vep.ann.vcf$",files)
write(paste("HC normal file: ",haplotypecaller_N_file),stdout())

# snpEff+VEP Mutect2 file
# mutect2_file <- grep(pattern = "^.*Annotation/VEP/mutect2_.*[TR]_vs_.*[BN]\\.vcf.*vcf$",files,value = T)[1]
mutect2_file <- checkFileExistence(".*mutect2_.*AF.*vep.ann.vcf$",files)
write(paste("Mutect2 file: ",mutect2_file),stdout())

# snpEff + VEP Strelka snv file
strelka_snv_file <- checkFileExistence(".*Strelka_.*_somatic_snvs.*AF.*vep.ann.vcf$",files)
write(paste("Strelka SNVs file: ",strelka_snv_file),stdout())
# snpEff + VEP Strelka indel file
strelka_indel_file <- checkFileExistence(".*Strelka_.*_somatic_indels.*AF.*vep.ann.vcf$",files)
write(paste("Strelka indels file: ",strelka_indel_file),stdout())
...
### Load dependencies
<!-- Dependencies -->
```{r load_dependencies and command line options}
write("##########################################################",stderr())
# use str(knitr::opts_knit$get()) to get knitr options
write("Run parameters: ",stderr())

ref_data <- params$reference_data
write(paste("Reference directory: ",ref_data),stderr())

sample_dir <- knitr::opts_knit$get("root.dir")
write(paste("Sample dir: ",sample_dir),stderr())
...
<!-- Reference data files TODO: serialize somehow -->
```{r load_reference}
snp_data = paste(ref_data,'private/swegen_snp_counts.csv',sep="")
write(paste("Loading SNP data ",snp_data),stderr());
snptable         = fread( snp_data,key='name')

sv_data = paste(ref_data,'private/swegen_sv_counts.csv',sep="")
write(paste("Loading structural variants data ",sv_data),stderr());
swegen_manta_all = fread(sv_data,key='name')

#allgenes        = fread('reference_data/genes_ensemble_canonical.csv',key='name')
...
hotspots_inframe_data = paste(ref_data,'hotspots_v2_inframe.csv',sep='')
hotspots_snv_data = paste(ref_data,'hotspots_v2_snv.csv',sep='')

write(paste("Reading in frame hotspots from ",hotspots_inframe_data, "and", hotspots_snv_data),stderr())
hotspots_inframe=unique(fread(hotspots_inframe_data)[,.(Hugo_Symbol,Amino_Acid_Position)])
...
hotspots_snv=unique(
  fread(hotspots_snv_data)[,.(Hugo_Symbol,Amino_Acid_Position)])[-grep('splice',Amino_Acid_Position)]
...
# loading COSMIC stuff

cosmic_coding_data    = paste(ref_data,'cosmic_coding_table.csv',sep='')
cosmic_noncoding_data = paste(ref_data,'cosmic_noncoding_table.csv',sep='')
cosmic_fusions_data   = paste(ref_data,'cosmic_fusions_table.csv',sep='')
write(paste("Reading COSMIC data from ",cosmic_coding_data, ",", cosmic_noncoding_data, "and", cosmic_fusions_data),stderr())
cosmic_coding    = fread(cosmic_coding_data,key = 'name')
cosmic_noncoding = fread(cosmic_noncoding_data,key = 'name')
cosmic_fusions   = fread(cosmic_fusions_data,key = 'name')
```
...
```{r load_control_freec}
  ...
  if (!is.na(freec_Tratio_file[1])) {
    ...
    write("Extract sample names, Tratio and Tbaf",stderr());
    ...
    # normal log ratio
    write("Normal log ration and BAF",stderr());
...
#allpass=NULL
#vcfs=list()
#write("Collecting PASS ids from all samples",stderr())
...
#}
#
##write(paste("VCFS",vcfs),stdout())
##getAllPass <- function(vcfFiles, aVCF) {
##  if (length(aVCF)>0) for (s in 1:length(aVCF)) {
##    vcfFiles[[aVCF[s]]]=readVcf(file = aVCF[s],genome = reference_genome)
##    pass=rowRanges(vcfFiles[[aVCF[s]]])$FILTER=='PASS'
##    names(vcfFiles[[aVCF[s]]])[pass]
##  }
##}
#
## then collect variants...
#write("Collecting variants",stderr())
#if (length(manta_normal_file)>0) for (s in 1:length(manta_normal_file)) {
...
#    
#  # make table of variants
#  write("Make a table of variants",stderr())
#  sv_table=cbind(
#    data.table(ID=as.character(names(vcf)),
#               sample,
#               chr=as.character(seqnames(rr))),
...
#  
#  # Filter out variants seen 2+ (?) times in reference data
#  write("Filtering out duplicates",stderr())
#  ## Key has only chr,start,end
#  key=sv_table[,c('chr','start','end')]
...
#    
#  # put in Swegen count
#  write("Add SweGen counts",stderr())
#  sv_table$Swegen_count=swegen_manta_all[key,value]
#  sv_table$Swegen_count[is.na(sv_table$Swegen_count)]=0
#  ## do the filter
#  sv_table <- sv_table[Swegen_count<2]
...
#    }
#    
#    write("Add snpEff annotations",stderr())
#    # snpEff annotation is in the ANN column.
#    h=strsplit(info(header(vcf))['ANN',][,3],'annotations: \'')[[1]][2]
#    snpEff_header=trimws(strsplit(h,'\\|')[[1]])
#      
...
#    
#    # Filter out annotations of certain types where the ID has >N annotations of that type
#    write("Filter annotations",stderr())
#    ids=unique(snpEff_table$ID)
#    for (id in ids) {
#      common=c('protein_protein_contact', 'duplication', 'structural_interaction_variant', 'inversion',
#               'transcript_ablation', 'feature_ablation', 'sequence_feature', 
...
#  if (nrow(selection)>0) {
#    # Known cancer genes affect ranking by +1
#    write("Known cancer genes",stderr());
#    for (gene in unique(selection$Gene_Name)) if (!is.na(gene)) if (gene!='') {
#      ix=selection$Gene_Name==gene
#      gene=sort(strsplit(gene,'&')[[1]])
#      # cosmic/local Tier2:
...
#    }
#    # Add moderate impact
#    write("Add moderate impact",stderr())
#    ix=selection$Annotation_Impact=='MODERATE'
#    if (any(ix)) {
#      selection$rank_score[ix]=selection$rank_score[ix]+1
#      selection$rank_terms[ix]=paste(selection$rank_terms[ix],'moderate_impact')
...
#    
#    # cosmic_>xx, fusion_gene or just fusion
#    write("Cosmic fusion genes",stderr())
#    ix=grep('fusion',selection$Annotation)
#    if (length(ix)>0) for (i in ix) if (selection$Gene_Name[i]!='') {
#      gene=sort(strsplit(selection$Gene_Name[i],'&')[[1]])
#      if (paste(gene,collapse = ' ') %in% cosmic_fusions[value>5,name]) {
...
#    
#    # -1 for ablation if long del
#    write("Long DEL ablation",stderr())
#    ix=intersect(
#      which(selection$SVTYPE=='DEL' & selection$chr==selection$altchr & abs(selection$altpos-selection$start) > 3e6),
#      grep('ablation',selection$Annotation)
#    )
...
#    write("LOH by ControlFREEC",stderr())
#    try( {
...
if (project=='BTB') {
  ...
  if (!is.null(manta_normal_table)) if (nrow(manta_normal_table)>0) {
    ...
    if (nrow(selection)>0) {
      ...
      write("Selecting ",stderr());
      manta_normal_selected <- selection[order(Feature_ID)][order(rank_score,decreasing = T)]
      write("Wringing  file ",stderr());
      fwrite(manta_normal_selected[,-c('ANN','Gene_ID')][rank_score>1],file=paste0(sampleData$name,'_manta_normal.csv'))
      write("Table wrapper ",stderr());
      tableWrapper(manta_normal_selected[,-c('ANN','Gene_ID')][rank_score>1])
      write("Ready. ",stderr());
    } 
  }
...
write("Plotting structural variants.",stderr());
...
if (exists('mutect2_table')) for (sample in sort(unique(mutect2_table$sample))) {
  ...
  g_mutect2=g_basic+
    ...
    geom_label_repel(data = temp[SYMBOL %in% alltumorgenes & Amino_acids!='',],
...
chromplot=function(thischr) if (T) {
  ...
  for (s in 1:nsamples) {
    ...
    temp=unique(strelka_table[samples[s]][chr==thischr & type!='other',][,.(start,AFreq,type,Amino_acids,SYMBOL,Protein_position)])
    if (nrow(temp)>0) {
      ...
      # with labels
      g=g+geom_label_repel(data = temp[Amino_acids!='' & SYMBOL %in% alltumorgenes,],
