...
if (project == 'BTB') {
  ...
  manta_normal_file <- checkFileExistence(".*Manta_.*vs.*somaticSV.vcf.snpEff.ann.vep.ann.vcf$", files)
...
if (project=='BTB') {
  ...
  if (length(manta_normal_file)>0) for (s in 1:length(manta_normal_file)) {
    ...
    # Read counts
    srtable=as.data.table(g$SR)
    bugger=paste0(SplitReadSupport_',c('N')) # Verified on a test sample
    write(paste0("bugger ",bugger),stderr())
    colnames(srtable)=bugger
    prtable=as.data.table(g$PR)
